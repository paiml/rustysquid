...
async fn test_cache_control_headers() {
    let test_cases = vec![
        ...
        (vec!["Cache-Control: max-age=3600".to_string()], true, "max-age"),
        (vec!["Cache-Control: no-cache".to_string()], false, "no-cache"),
        (vec!["Cache-Control: no-store".to_string()], false, "no-store"),
        (vec!["Cache-Control: private".to_string()], false, "private"),
        (vec!["Cache-Control: private, max-age=3600".to_string()], false, "private with max-age"),
    ];
...
fn test_ttl_calculations() {
    ...
    assert_eq!(calculate_ttl(&["Cache-Control: max-age=60".to_string()]), 60);
    assert_eq!(calculate_ttl(&["Cache-Control: max-age=3600".to_string()]), 3600);
    assert_eq!(calculate_ttl(&["Cache-Control: max-age=86400".to_string()]), 86400);
    
    // Should cap at 24 hours
    assert_eq!(calculate_ttl(&["Cache-Control: max-age=100000".to_string()]), 86400);
    
    // Invalid max-age should use default
    assert_eq!(calculate_ttl(&["Cache-Control: max-age=invalid".to_string()]), CACHE_TTL);
}
...
async fn test_cache_memory_limits() {
    ...
    assert!(!cache.put(1, oversized).await, "Should reject oversized entry");
    ...
    assert!(cache.put(2, max_allowed).await, "Should accept max-size entry");
