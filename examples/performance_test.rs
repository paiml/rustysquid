...
use rustysquid::{ProxyCache, CachedResponse, create_cache_key, MAX_ENTRY_SIZE};
use bytes::Bytes;
...
async fn test_sequential_performance(cache: &ProxyCache) {
    ...
    println!("      Throughput: {:.0} ops/sec", iterations as f64 / total_put.as_secs_f64());
    
    println!("   GET operations:");
    println!("      Total: {:?}", total_get);
    println!("      Average: {:?}", total_get / iterations as u32);
    println!("      Throughput: {:.0} ops/sec", iterations as f64 / total_get.as_secs_f64());
